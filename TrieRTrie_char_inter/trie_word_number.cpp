...
// chercher le chemin dans le trie et sauvgarder les node que on a passer par jusqu'à la fin ou la position d'erreur.
...
// ci la meme fonction que la precedent saufe que on sauvgarde les node dans "RT_node_way", bon je sais que je peut faire sa dans la meme fonction avec des pointeur à l'enrtie de la fonction
...
// l'erreur est à la fin (example : ABC*) , dc on est dans le node C
// on a :
...
void insert_after_exact(node* node_in)
{
    ...
    // tous les fils de node (D) sont des resultat de insertion , sa resemble à un substitution avec un mot de taille 5 example(abcd* et on trouve abcdx)
    for(i=0; i<NB_CHAR;i++)
    {
        if(node_in->tab[i]!=NULL && node_in->tab[i]->nb_word!=-1)
        {
...
void approximate_search(Trie_R_Trie* my_TRT,char *queryWord, int size_queryWord)
{
    ...
    // ------------------------------------------------------------------------------------------------
    // ------------------------------------------------------------------------------------------------
    //  traiter le cas où on trouve un mot exact (ou queryword est sous mot d'un autre mot dans le trie)

    // si on trouve un mot exact dans le trie : ABCD
    if(idx_node_way>size_queryWord) // car idx_node_way represent la taille de tableau de node_way et node_way[0]=racine, node_way[1]=1er char ...., idx_node_way est tj dans la case vide après les char. dc ci size(ABCD)=4, idx_node_way=5
    {
